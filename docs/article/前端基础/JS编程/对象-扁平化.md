...
function transform(entryObj) {
    ...
    // leftKeys 包含key字符的数组，每次迭代都会减少
    // keyStr 包含key字符的字符串，每次迭代都会增大，吸收leftKeys中的值，最终leftKeys为空时，通过此字段为entryObj提供key
    function fn(outputObj, leftKeys, keyStr='') {
        let currKey = leftKeys.shift();
        keyStr += keyStr ? ('.' + currKey) : currKey; // 1、entryObj key的构建（用来获取value）

        if (leftKeys.length === 0) { // 2、keys为0，说明keys已遍历完成，进行赋值操作
            outputObj[currKey] = entryObj[keyStr]; //
        } else { // 3、keys不为0，说明keys尚未遍历完成，需继续遍历，同时根据遍历的key值构建结构
            if (!outputObj[currKey]) outputObj[currKey] = {};
            fn(outputObj[currKey], leftKeys, keyStr) // 深入嵌套
        }
    }
}
